...
Status read_and_validate_decode_args(char *argv[], decodeInfo *decInfo)
{
    ...
    else
    {
        ...
        usleep(1000000);
        printf("Defaulty create file %s",decInfo->extn_fname);
	}
return e_success;
}
...
{
    int index=0;
    char decode_extention[index];//Extension 
    //printf("%d\n",deInfo->file_size);
    for(int i=0;i<deInfo->file_size;i++)
    {
        char buffer[8];
        fread(buffer,1,8,deInfo-> fptr_image);
        decode_extention[index++]=decode_byte_from_lsb(buffer);//we are storing extension in an array
    }
    decode_extention[index]='\0';
    printf("extention:%s\n",decode_extention);
    strcat(deInfo->extn_fname,decode_extention);//concecnate extension and file name
    printf("After concecante: %s\n",deInfo->extn_fname);
    return e_success;
}
